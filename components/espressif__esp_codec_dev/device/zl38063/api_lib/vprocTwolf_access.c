 ...
 * page, offset and build the 16-bit command accordingly to the access type.
...
/*The following 3 functions provide a mean to loading the *.s3 firmware into
 * the device
 * - Call sequence:
...
static int spisTwBootWrite(char *blockOfFwrData)  /*0: HBI; 1:FLASH*/
{
    ...
    if (blockOfFwrData == NULL) {
        DEBUG_LOGE(TAG_SPI, "blockOfFwrData = %s\n", blockOfFwrData);
        return VPROC_STATUS_INVALID_ARG;
    }
    ...
    if ((rec_type == 7) || (rec_type == 8) || (rec_type == 9)) {
        ...
        return TWOLF_STATUS_BOOT_COMPLETE;  /*BOOT_COMPLETE Successfully*/
    }
...
VprocStatusType VprocTwolfSaveImgToFlash(void)
{
    ...
    /*check whether the actions above were performed successfully*/
...
VprocStatusType VprocTwolfSaveCfgToFlash(void)
{
    ...
    /* Clear the checksum register so that the firmware can calculate a new chk value
    ...
    if ((buf > 0)) {
        ...
        /*Verify whether the operation completed successfully*/
