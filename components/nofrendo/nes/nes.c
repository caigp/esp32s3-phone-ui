...
{
   log_printf("%s *machine = %p", __func__, *machine);
   if (*machine)
   {
...
_fail:
   log_printf("nes_insertcart machine = %p", (*machine));
   nes_destroy(machine);
   return -1;
}
...
nes_t *nes_create(void)
{
   ...
   machine = heap_caps_malloc(sizeof(nes_t), MALLOC_CAP_SPIRAM);
   if (NULL == machine)
      return NULL;
...
//      goto _fail;
   ...
   /* cpu */
   machine->cpu = heap_caps_malloc(sizeof(nes6502_context), MALLOC_CAP_SPIRAM);
   if (NULL == machine->cpu)
      goto _fail;
   ...
   /* allocate 2kB RAM */
   machine->cpu->mem_page[0] = heap_caps_malloc(NES_RAMSIZE, MALLOC_CAP_SPIRAM);
...
_fail:
   log_printf("nes_create machine = %p", machine);
   nes_destroy(&machine);
   return NULL;
}
