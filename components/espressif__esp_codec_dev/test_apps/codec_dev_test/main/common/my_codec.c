 ...
 */
static int my_codec_open(const audio_hw_base_t *h, void *cfg, int cfg_size)
{
...
static bool my_codec_is_open(const audio_hw_base_t *h)
{
    my_codec_t *codec = (my_codec_t *)h;
...
static int my_codec_set_fs(const audio_hw_base_t *h, esp_codec_dev_sample_info_t *fs, esp_codec_dev_type_t type)
{
    (void)type;
    my_codec_t *codec = (my_codec_t *)h;
...
static int my_codec_dac_mute(const audio_codec_if_t *h, int ch_mask, bool mute)
{
    my_codec_t *codec = (my_codec_t *)h;
...
static int my_codec_dac_set_vol(const audio_codec_if_t *h, int ch_mask, float db)
{
    my_codec_t *codec = (my_codec_t *)h;
...
static int my_codec_adc_set_vol(const audio_codec_if_t *h, int ch_mask, float db)
{
    my_codec_t *codec = (my_codec_t *)h;
...
static int my_codec_adc_mute(const audio_codec_if_t *h, int ch_mask, bool mute)
{
    my_codec_t *codec = (my_codec_t *)h;
...
static int my_codec_set_reg(const audio_hw_base_t *h, int reg, int value)
{
    my_codec_t *codec = (my_codec_t *)h;
...
static int my_codec_get_reg(const audio_hw_base_t *h, int reg, int *value)
{
    my_codec_t *codec = (my_codec_t *)h;
    if (reg < MY_CODEC_REG_MAX) {
        *value = 0;
...
static int my_codec_close(const audio_hw_base_t *h)
{
    my_codec_t *codec = (my_codec_t *)h;
    // Auto disable when codec closed
    if (codec->enable) {
        my_codec_enable((const audio_codec_if_t *)h, false);
    }
    codec->is_open = false;
    return 0;
}
...
const audio_codec_if_t *my_codec_new(my_codec_cfg_t *codec_cfg)
{
    ...
    codec->base.hw_base.open = my_codec_open;
    codec->base.hw_base.is_open = my_codec_is_open;
    codec->base.hw_base.set_fs = my_codec_set_fs;
    codec->base.hw_base.set_reg = my_codec_set_reg;
    codec->base.hw_base.get_reg = my_codec_get_reg;
    codec->base.hw_base.close = my_codec_close;
    codec->base.ctrl_if = codec_cfg->ctrl_if;

    // Initialize ADC operations
    codec->adc_ops.ops.enable = my_codec_enable;
    codec->adc_ops.ops.mute = my_codec_adc_mute;
    codec->adc_ops.ops.set_vol = my_codec_adc_set_vol;
    codec->base.adc_if = &codec->adc_ops;

    // Initialize DAC operations
    codec->dac_ops.ops.enable = my_codec_enable;
    codec->dac_ops.ops.mute = my_codec_dac_mute;
    codec->dac_ops.ops.set_vol = my_codec_dac_set_vol;
    codec->base.dac_if = &codec->dac_ops;
    codec->base.hw_proc = &my_codec_hw_proc;

    // Calculate hardware gain from configuration
    codec->hw_gain = esp_codec_dev_vol_calc_hw_gain(&codec_cfg->hw_gain);
    codec->base.hw_base.open(&codec->base.hw_base, codec_cfg, sizeof(my_codec_cfg_t));
    return &codec->base;
}
...
const my_codec_proc_state_t *my_codec_get_proc_state(const audio_codec_if_t *codec_if)
{
    ...
    my_codec_t *codec = (my_codec_t *)codec_if;
    return &codec->proc;
}
...
my_codec_proc_state_t *my_codec_get_mutable_proc_state(const audio_hw_base_t *hw_base)
{
    ...
    my_codec_t *codec = (my_codec_t *)hw_base;
    return &codec->proc;
}
