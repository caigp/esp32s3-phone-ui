                              ...
                              int height, int pitch, int overdraw)
{
   ...
   bitmap = heap_caps_malloc(sizeof(bitmap_t) + (sizeof(uint8 *) * height), MALLOC_CAP_SPIRAM);
   ...
   // printf("bitmap pitch %d, bitmap->hardware", bitmap->pitch, bitmap->hardware);
...
bitmap_t *bmp_create(int width, int height, int overdraw)
{
   ...
   addr = heap_caps_malloc((pitch * height) + 3, MALLOC_CAP_SPIRAM); /* add max 32-bit aligned adjustment */
   if (NULL == addr)
      return NULL;
