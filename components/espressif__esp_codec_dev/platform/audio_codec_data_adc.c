...
static const char *TAG = "CODEC_ADC_IF";
...
#if ESP_IDF_VERSION >= ESP_IDF_VERSION_VAL(5, 5, 2)
    adc_continuous_data_t            *sdk_parsed_buf;  /* Preallocated for adc_continuous_parse_data() */
#endif  /* ESP_IDF_VERSION >= ESP_IDF_VERSION_VAL(5, 5, 2) */
                                    ...
                                    uint32_t *num_parsed_samples)
{
    ...
    if (sample_count > adc_data->parsed_buf_count) {
        *num_parsed_samples = 0;
        return ESP_ERR_INVALID_SIZE;
    }
#if ESP_IDF_VERSION >= ESP_IDF_VERSION_VAL(5, 5, 2)
    if (adc_data->sdk_parsed_buf == NULL) {
        *num_parsed_samples = 0;
        return ESP_ERR_INVALID_SIZE;
    }
    esp_err_t ret = adc_continuous_parse_data(adc_data->handle, raw_data, raw_data_size,
                                              adc_data->sdk_parsed_buf, num_parsed_samples);
    if (ret != ESP_OK) {
        return ret;
    }
    for (uint32_t i = 0; i < *num_parsed_samples; i++) {
        parsed_data[i].unit = adc_data->sdk_parsed_buf[i].unit;
        parsed_data[i].channel = adc_data->sdk_parsed_buf[i].channel;
        parsed_data[i].raw_data = adc_data->sdk_parsed_buf[i].raw_data;
        parsed_data[i].valid = adc_data->sdk_parsed_buf[i].valid;
    }
#else
    for (uint32_t i = 0; i < sample_count; i++) {
        ...
        if (adc_data->cfg.format == ADC_DIGI_OUTPUT_FORMAT_TYPE2) {
            ...
            /* TYPE1 carries no unit bit, so recover it from the active pattern table. */
...
#if ESP_IDF_VERSION >= ESP_IDF_VERSION_VAL(5, 5, 2)
    if (adc_data->sdk_parsed_buf) {
        free(adc_data->sdk_parsed_buf);
        adc_data->sdk_parsed_buf = NULL;
    }
#endif  /* ESP_IDF_VERSION >= ESP_IDF_VERSION_VAL(5, 5, 2) */
...
const audio_codec_data_if_t *audio_codec_new_adc_data(audio_codec_adc_cfg_t *adc_cfg)
{
    ...
    ESP_RETURN_ON_FALSE(adc_cfg->continuous_cfg.pattern_num <= SOC_ADC_PATT_LEN_MAX,
                        NULL, TAG, "Pattern_num too large");
    ...
    adc_data->parsed_buf_count = adc_data->raw_read_buf_size / SOC_ADC_DIGI_RESULT_BYTES;
    if (adc_data->parsed_buf_count == 0) {
        adc_data->parsed_buf_count = 1;
    }
    adc_data->parsed_buf = calloc(adc_data->parsed_buf_count, sizeof(codec_adc_parsed_data_t));
...
#if ESP_IDF_VERSION >= ESP_IDF_VERSION_VAL(5, 5, 2)
    adc_data->sdk_parsed_buf = calloc(adc_data->parsed_buf_count, sizeof(adc_continuous_data_t));
    if (adc_data->sdk_parsed_buf == NULL) {
        free(adc_data->parsed_buf);
        free(adc_data->raw_read_buf);
        free(adc_data);
